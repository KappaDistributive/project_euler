...
int main() {
  ...
  for (size_t y{0}; y < 20; y++) {
    for (size_t x{0}; x + 3 < 20; x++) {
      size_t candidate{1};
      for (size_t offset{0}; offset < 4; offset++) {
        candidate *= input[y * 20 + x + offset];
      }
      result = std::max(result, candidate);
    }
  }
  // vertical
  for (size_t y{0}; y + 3 < 20; y++) {
    for (size_t x{0}; x < 20; x++) {
      size_t candidate{1};
      for (size_t offset{0}; offset < 4; offset++) {
        candidate *= input[(y + offset) * 20 + x];
      }
      result = std::max(result, candidate);
    }
  }
  ...
  for (size_t y{0}; y < 20; y++) {
    for (size_t x{0}; x < 20; x++) {
      size_t candidate{1};
      for (size_t offset{0}; offset < 4; offset++) {
        if (x + offset >= 20 || y + offset >= 20) {
          candidate = 0;
          break;
        }
        candidate *= input[(y + offset) * 20 + (x + offset)];
      }
      result = std::max(result, candidate);
    }
  }
